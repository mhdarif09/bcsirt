...
if ( ! function_exists( 'exs_filter_post_class' ) ) :
	function exs_filter_post_class( $exs_classes, $additional_class, $post_id ) {
		...
		if ( is_single() && empty( $exs_special_slug ) && ( 'title-section-image' === exs_option( 'blog_single_layout', '' ) || '6' === exs_option( 'title', '' ) || '7' === exs_option( 'title', '' ) || '8' === exs_option( 'title', '' ) ) ) {
	...
	function exs_filter_the_content_add_toc_in_post( $html ) {
		...
		if ( ! empty( $h ) ) :
			...
			foreach ( $matches[2] as $key => $title ) {
				...
				if ( in_array( $key, $manual_ids ) ) {
					if (preg_match( '/id="(.*)"/', $matches[0][ $key ], $matches_id)) {
						$link_hash = $matches_id[1];
					} else {
						$link_hash = '';
					}
				} else {
					$link_hash = sanitize_title( strip_tags( $title ) );
				}

				$toc_html .= '<li style="margin-left:' . esc_attr( $margin_left ) . 'em" class="' . esc_attr( $li_class ) . '"><a class="exs-toc-item" href="#' .  $link_hash . '">' . strip_tags( $title ) . '</a></li>';
			}
			$toc_html .= '</ul></nav></div>';
		endif; //! empty $h
