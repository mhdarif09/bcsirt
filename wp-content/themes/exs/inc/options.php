	...
	function exs_get_feed_layout_options( $exs_category = false ) {
		...
		$exs_return = apply_filters(
			...
			array(
				...
				'meta-bottom 2'                   => esc_html__( 'Bottom post meta - 2 columns', 'exs' ),
				'meta-bottom 3'                   => esc_html__( 'Bottom post meta - 3 columns', 'exs' ),
				'meta-bottom 4'                   => esc_html__( 'Bottom post meta - 4 columns', 'exs' ),
				'meta-image-bottom'               => esc_html__( 'Bottom image and post meta', 'exs' ),
				'meta-image-bottom 2'             => esc_html__( 'Bottom image and post meta - 2 columns', 'exs' ),
				'meta-image-bottom 3'             => esc_html__( 'Bottom image and post meta - 3 columns', 'exs' ),
				'meta-image-bottom 4'             => esc_html__( 'Bottom image and post meta - 4 columns', 'exs' ),
...
if ( ! function_exists( 'exs_get_typography_inline_styles_string' ) ) :
	function exs_get_typography_inline_styles_string() {
		...
		$headings_array = array(
			...
			'h5' => 'h5',
			'h6' => 'h6',
			'post_title' => '.hfeed #layout .entry-title',
			'post_title_single' => '.single #layout .entry-title',
		);
		foreach( $headings_array as $h => $selector ) {
			$h_string = '';
			$h_string .= ! empty( exs_option( 'typo_size_' . $h ) ) ? 'font-size:' . (float) exs_option( 'typo_size_' . $h ) . 'em;' : '';
			$h_string .= ! empty( exs_option( 'typo_line_height_' . $h ) ) ? 'line-height:' . (float) exs_option( 'typo_line_height_' . $h ) . 'em;' : '';
			$h_string .= ! empty( exs_option( 'typo_letter_spacing_' . $h ) ) ? 'letter-spacing:' . exs_option( 'typo_letter_spacing_' . $h ) .'em;' : '';
			$h_string .= ! empty( exs_option( 'typo_weight_' . $h ) ) ? 'font-weight:' . (int) exs_option( 'typo_weight_' . $h ) . ';' : '';
			$h_string .= ! empty( exs_option( 'typo_mt_' . $h ) ) ? 'margin-top:' . (float) exs_option( 'typo_mt_' . $h ) . 'em;': '';
			$h_string .= ! empty( exs_option( 'typo_mb_' . $h ) ) ? 'margin-bottom:' . (float) exs_option( 'typo_mb_' . $h ) . 'em;' : '';

			if ( $h_string ) {
				$exs_typography_string .= $selector . '{' . $h_string . '}';
			}
		}
...
//get singular post types to display reading progress bard (print markup and enqueue script)
if ( ! function_exists( 'exs_get_post_singular_post_types_for_reading_progress_bar' ) ) :
	function exs_get_post_singular_post_types_for_reading_progress_bar() {
		$post_types = array( 'post' );
		$extra_post_types_string = trim( exs_option('blog_single_read_progress_bar_posttypes', '' ) );
		if ( ! empty( $extra_post_types_string ) ) {
			$extra_post_types_array = explode(',', $extra_post_types_string );
			foreach ( $extra_post_types_array as $post_type_slug ) {
				$post_types[] = trim( $post_type_slug );
			}
		}
		return $post_types;
	}
